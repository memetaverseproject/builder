...
export const PayPublicationFeeStep: React.FC<MapStateProps & { onNextStep: () => void; onPrevStep: () => void }> = props => {
  ...
  if (refRarity) {
    ...
    totalPrice = ethers.BigNumber.from(refRarity.prices!.MTV).mul(items.length).toString()

    totalPriceUSD = ethers.BigNumber.from(priceUSD).mul(items.length).toString()

    hasInsufficientMANA = !!wallet && wallet.networks.U2U.mana < Number(ethers.utils.formatEther(totalPrice))
  }
      ...
      return (
        <small className="not-enough-mana-notice error">
          {t('publish_collection_modal_with_oracle.not_enough_mana', {
            symbol: (
              <span>
                <Mtv network={Network.U2U} inline /> Mtv
              </span>
            )
          })}
          <br />
