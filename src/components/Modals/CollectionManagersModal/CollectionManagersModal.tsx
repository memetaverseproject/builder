...
import { ModalNavigation, ModalActions } from '@mtvproject/ui'
    ...
    return (
      <Modal className="CollectionManagersModal" onClose={onClose}>
        ...
        <Modal.Content>
          ...
          {managers.length ? (
            <ModalActions>
              <NetworkButton primary onClick={this.handleSubmit} loading={isLoading} disabled={this.isDisabled()} network={Network.U2U}>
                {t('global.add')}
              </NetworkButton>
            </ModalActions>
          ) : null}
