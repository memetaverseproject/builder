...
import { Section, Row, Narrow, Column, Header, Button, Popup, Tabs, Table, Label, SemanticSIZES } from '@mtvproject/ui'
import { NetworkCheck } from '@mtvproject/dapps/dist/containers'
...
export default class CollectionDetailPage extends React.PureComponent<Props, State> {
  ...
  renderActionButtoms(items: Item[]) {
    ...
    return (
      <>
        <JumpIn size="small" active collection={collection} text={t('global.see_in_memetaverse')} disabled={isLocked || !hasItems} />
  ...
  renderToggleOnSaleButtom() {
    ...
    return (
      <NetworkCheck network={Network.U2U}>
        {isEnabled => (
          <Button
