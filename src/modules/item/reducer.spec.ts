...
describe('when reducing an action of a successful rescue items', () => {
  ...
  it('should remove the rescue items request action from the loading array and null the error', () => {
    expect(itemReducer(state, rescueItemsSuccess(collection, items, contentHashes, ChainId.U2U_NEBULAS, ['hashes']))).toEqual({
      ...state,
      loading: [],
      error: null
    })
...
describe('when reducing an action of a successful chunk of rescued items', () => {
  ...
  it('should add the items to the state', () => {
    expect(itemReducer(state, rescueItemsChunkSuccess(collection, items, contentHashes, ChainId.U2U_NEBULAS, 'hash'))).toEqual({
      ...state,
      data: {
        ...state.data,
        [items[0].id]: items[0]
