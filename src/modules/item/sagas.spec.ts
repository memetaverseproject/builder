...
import { Entity, EntityType } from '@mtvproject/schemas'
import { call, select, take, race, delay } from 'redux-saga/effects'
import { BuilderClient, RemoteItem } from '@mtvproject/builder-client'
import { ChainId, Network, BodyShape, WearableCategory } from '@mtvproject/schemas'
import { ToastProps, ToastType } from '@mtvproject/ui'
import { Toast } from '@mtvproject/dapps/dist/modules/toast/types'
...
import { FETCH_TRANSACTION_FAILURE, FETCH_TRANSACTION_SUCCESS } from '@mtvproject/dapps/dist/modules/transaction/actions'
...
import { SHOW_TOAST } from '@mtvproject/dapps/dist/modules/toast/actions'
    ...
    it('should put a setPriceAndBeneficiarySuccess action', async () => {
      ...
      await expectSaga(itemSaga, builderAPI, builderClient)
        .provide([
          ...
          [call(getChainIdByNetwork, Network.U2U), ChainId.U2U_MAINNET],
          [matchers.call.fn(sendTransaction), Promise.resolve('0xhash')]
        ])
        .put(setPriceAndBeneficiarySuccess({ ...item, price, beneficiary }, ChainId.U2U_MAINNET, '0xhash'))
...
describe('when handling the rescue items request action', () => {
  ...
  describe('and the meta transactions are successful', () => {
    it('should dispatch a rescueItemsChunkSuccess per chunk and the rescueItemsSuccess once the transactions finish', () => {
      return expectSaga(itemSaga, builderAPI, builderClient)
        .provide([
          [call(getChainIdByNetwork, Network.U2U), ChainId.U2U_NEBULAS],
        ...
        .put(rescueItemsChunkSuccess(collection, groupsOfItems[0], groupsOfContentHashes[0], ChainId.U2U_NEBULAS, txHash))
        .put(rescueItemsChunkSuccess(collection, groupsOfItems[1], groupsOfContentHashes[1], ChainId.U2U_NEBULAS, txHash))
        .put(rescueItemsSuccess(collection, resultItems, contentHashes, ChainId.U2U_NEBULAS, [txHash, txHash]))
