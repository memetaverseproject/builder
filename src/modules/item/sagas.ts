...
import { ChainId, Network, Entity, EntityType } from '@mtvproject/schemas'
...
import { closeAllModals, closeModal } from '@mtvproject/dapps/dist/modules/modal/actions'
...
import { RENDER_TOAST, hideToast, showToast, RenderToastAction } from '@mtvproject/dapps/dist/modules/toast/actions'
import { ToastType } from '@mtvproject/ui'
...
export function* itemSaga(legacyBuilder: LegacyBuilderAPI, builder: BuilderClient) {
  ...
  function* handleSetPriceAndBeneficiaryRequest(action: SetPriceAndBeneficiaryRequestAction) {
    ...
    try {
      ...
      const chainId: ChainId = yield call(getChainIdByNetwork, Network.U2U)
      const contract = { ...getContract(ContractName.ERC721Collection, chainId), address: collection.contractAddress! }
  ...
  function* handleRescueItemsRequest(action: RescueItemsRequestAction) {
    ...
    try {
      ...
      const { abi } = getContract(ContractName.ERC721Collection, chainId)
