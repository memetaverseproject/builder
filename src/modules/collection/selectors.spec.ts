import { ChainId, WearableCategory } from '@mtvproject/schemas'
...
describe('when getting status by item id', () => {
  it('should return the status for each published item', () => {
    mockGetChainIdByNetwork.mockReturnValue(ChainId.U2U_MAINNET)
    const mockState = {
      collectionCuration: {
        data: {
          '1': {
...
describe('when getting the rarities contract', () => {
  ...
  describe('when the provided chain id is mumbai', () => {
    beforeEach(() => {
      providedChainId = ChainId.U2U_NEBULAS
    })

    it('should return the RaritiesWithOracle contract for mumbai', () => {
      expect(getRaritiesContract(providedChainId)).toEqual(getContract(ContractName.Rarities, ChainId.U2U_NEBULAS))
    })
  })

  describe('when the provided chain id is matic mainnet', () => {
    beforeEach(() => {
      providedChainId = ChainId.U2U_MAINNET
    })

    it('should return the RaritiesWithOracle contract for matic mainnet', () => {
      expect(getRaritiesContract(providedChainId)).toEqual(getContract(ContractName.Rarities, ChainId.U2U_MAINNET))
    })
  })
})
