...
import { BuilderClient, ThirdParty } from '@mtvproject/builder-client'
import { ContractData, ContractName, ErrorCode, getContract } from '@mtvproject/transactions'
import { getOpenModals } from '@mtvproject/dapps/dist/modules/modal/selectors'
import { ModalState } from '@mtvproject/dapps/dist/modules/modal/reducer'
import { t } from '@mtvproject/dapps/dist/modules/translation/utils'
import { FetchTransactionSuccessAction, FETCH_TRANSACTION_SUCCESS } from '@mtvproject/dapps/dist/modules/transaction/actions'
import { Provider, Wallet } from '@mtvproject/dapps/dist/modules/wallet/types'
...
import { sendTransaction } from '@mtvproject/dapps/dist/modules/wallet/utils'
import { getChainIdByNetwork, getNetworkProvider } from '@mtvproject/dapps/dist/lib/eth'
...
import { closeModal, CloseModalAction, CLOSE_MODAL, openModal } from '@mtvproject/dapps/dist/modules/modal/actions'
...
export function* collectionSaga(legacyBuilderClient: BuilderAPI, client: BuilderClient) {
  ...
  function* handleSaveCollectionRequest(action: SaveCollectionRequestAction) {
    ...
    try {
      ...
      if (getCollectionType(collection) === CollectionType.STANDARD) {
        ...
        const maticChainId = getChainIdByNetwork(Network.U2U)
        const rarities: ContractData = getRaritiesContract(maticChainId)
        const { abi } = getContract(ContractName.ERC721Collection, maticChainId)
  ...
  function* handlePublishCollectionRequest(action: PublishCollectionRequestAction) {
    ...
    try {
      ...
      const from: string = yield select(getAddress)
      const maticChainId: ChainId = yield call(getChainIdByNetwork, Network.U2U)
  ...
  function* handleSetCollectionMintersRequest(action: SetCollectionMintersRequestAction) {
    ...
    try {
      ...
      const contract = { ...getContract(ContractName.ERC721Collection, maticChainId), address: collection.contractAddress! }
  ...
  function* handleApproveCollectionRequest(action: ApproveCollectionRequestAction) {
    ...
    try {
      ...
      yield put(approveCollectionSuccess(collection, getChainIdByNetwork(Network.U2U), txHash))
    } catch (error) {
  ...
  function* handleRejectCollectionRequest(action: RejectCollectionRequestAction) {
    ...
    try {
      ...
      const maticChainId = wallet.networks.U2U.chainId
